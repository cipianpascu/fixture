...
History emission is disabled by default. When enabled, `ProxyService` and `SoapBackendService` emit events after auth/security enrichment and around the backend call for configured mutating methods.
    ...
    - name: orders
      ...
      history:
        ...
        additional-properties:
          ...
          customerIdFromHeaderOrToken: header:X-Customer-Id||token:partner_id|c_partner_id
          eventDate: "date: yyyy-MM-dd"
          eventTimestamp: date:timestamp
          appVersion: manifest:Implementation-Version
          requestId: header:X-Request-Id||generator:uuid
        generated-headers:
          X-Request-Id: generator:uuid
...
Applications provide the event body by implementing `HistoryPayloadMapper`. Use `supports(backend, incomingRequest, status)` to filter by backend, endpoint, method, or lifecycle status before payload construction. The mapper receives `HistoryRequestContext`, including the backend, incoming request, inbound/outbound body, outbound headers, resolved `additionalProperties`, lifecycle `status`, optional backend status code, and optional backend response body. Proxy services emit `SUBMITTED` before the backend call and `FULFILLED` or `FAILED` after the backend response. Publish attributes default to `additionalProperties`; override `HistoryPayloadMapper.attributes(context)` when a backend needs per-request attributes. `token:` values read JWT claims from `history.token-header`, defaulting to `Authorization`. `date:` values are generated in UTC and support `timestamp`, `epoch-second`, `iso-instant`, or Java date/time patterns. `manifest:` values read classpath manifest attributes, for example `manifest:Implementation-Version`. `generator:uuid` generates one UUID for an inbound request and reuses it for every lifecycle event. Use it directly in `additional-properties`, or configure it under `generated-headers` and reference that header from an attribute.
